...
router.post("/auth/signup", middleware.ensureNotLoggedIn, async (req,res) => {
	...
	const { firstName, lastName, email, cin, address, phone, password1, password2, role } = req.body;
	let errors = [];
	
	if (!firstName || !lastName || !email || !cin || !address || !phone ||!password1 || !password2) {
	...
	if(phone.length<10 || phone.length>10){
		errors.push({msg: "Phone length should be of 10 characters" });
	}
	// if(cin>21){
	// 	errors.push({msg: "Insert a valid CIN"});
	// }
	if(errors.length > 0) {
		return res.render("auth/signup", {
			title: "User Signup",
			errors, firstName, lastName, email,cin,address,phone, password1, password2
		});
	}
	...
	try
	{
		...
		if(user)
		{
			...
			return res.render("auth/signup", {
				title: "User Signup",
				firstName, lastName, errors, email, cin, address, phone, password1, password2
			});
		}
		
		const newUser = new User({ firstName, lastName, email, cin, address, phone, password:password1, role });
